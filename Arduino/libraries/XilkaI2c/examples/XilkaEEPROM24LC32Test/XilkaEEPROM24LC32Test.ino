...
void setup()
{
	...
	Wire.begin();
	Wire.setClock(Xilka::EEPROM24LC32::MaxI2CSpeed);

	// you may use an alternative address, e.g. storage.begin(0x51);
	if ( storage.begin() ) // default address is 0x50
	{
		Serial.println("Found EEPROM24LC32");
	}
	else
	{
		Serial.println("No EEPROM24LC32 found ... check your connections\r\n");
		while ( true )
			;
	}
}
